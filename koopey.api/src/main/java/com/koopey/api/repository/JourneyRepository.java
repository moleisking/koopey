...
public interface JourneyRepository extends BaseRepository<Journey, UUID>{
    ...
    @Query(nativeQuery = true, value = "SELECT Location.* FROM Journey J INNER JOIN Location L ON J.asset_id = L.asset_id WHERE asset_id = :asset_id")
    public List<Location> findAssetLocations (@Param("asset_id")UUID assetId);

    @Query(nativeQuery = true, value = "SELECT Location.* FROM Journey J INNER JOIN Location L ON J.game_id = L.asset_id WHERE asset_id = :asset_id")
