...
export class User {
    ...
    public static toHash(user: User): string {
        return SHA256(user.id + user.type + user.alias + user.name + user.birthday + user.currency + user.description +
            user.education + user.career + user.email + user.language + user.mobile + user.password + user.track + user.terms + user.cookies).toString();
    }
    ...
    public static isCreate(user: User): boolean {
        if (user.id &&
            ...
            user.email &&
            user.email.length >= 5 &&
            user.email.match("[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?") &&
            user.location &&
            user.name &&
            user.name.length >= 5 &&
            user.password &&
