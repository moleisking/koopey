...
export class Piece {
    ...
    constructor(color: PlayerType, type: PieceType, value: number, file: number, rank: number) {
        this.type = type;
        this.color = color;
        this.value = value;
        this.file = file;
    ...
    public static toHash(piece: Piece): string {
        return SHA256(piece.id + piece.type + piece.color + piece.value + piece.file + piece.moves + piece.rank).toString();
    }

    public static isEmpty(piece: Piece): boolean { 
        return ((piece == null) || (piece.type == null) || (piece.color == null)  || (piece.file == 0)  || (piece.rank == 0)) ? true : false;
    }
    ...
    public static clone(piece: Piece): Piece {
        var p: Piece = new Piece(piece.color, piece.type, piece.value, piece.file, piece.rank);
        p.id = piece.id;
        p.moves = piece.moves;
        return p;
    }
    ...
    public static equal(pieceA: Piece, pieceB: Piece): boolean {
        if ((pieceA.id == pieceB.id) && (pieceA.type == pieceB.type) && (pieceA.color == pieceB.color) && (pieceA.file == pieceB.file) && (pieceA.rank == pieceB.rank)) {
            return true;
        } else {
            return false;
        }
    ...
    public static equalPiece(pieceA: Piece, pieceB: Piece): boolean {
        if ((pieceA.id == pieceB.id) && (pieceA.type == pieceB.type) && (pieceA.color == pieceB.color)) {
            return true;
        } else {
            return false;
        }
