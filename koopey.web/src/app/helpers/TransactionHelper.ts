...
export class TransactionHelper {
    ...
    public static TotalTransactionValuePlusMargin(transaction: Transaction): number {
        if (transaction && transaction.currency && transaction.totalValue && transaction.totalValue > 0) {
            if (transaction.currency.match('btc|eth')) {
                return transaction.totalValue + (transaction.totalValue / 100) * Config.transaction_margin;
            } else {
                return 0;
            }
        }
    ...
    public static BuyerShareValue(transaction: Transaction): number {
        if (transaction && transaction.currency && transaction.users && transaction.totalValue && transaction.totalValue > 0) {
            return transaction.totalValue / this.CountBuyers(transaction);
        }
    }

    public static BuyerShareValuePlusMargin(transaction: Transaction): number {
        if (transaction && transaction.currency && transaction.users && transaction.totalValue && transaction.totalValue > 0) {
            var buyerShareValue = transaction.totalValue / this.CountBuyers(transaction);
            return buyerShareValue + (buyerShareValue / 100) * Config.transaction_margin;
        }
    }
    ...
    public static SellerShareValue(transaction: Transaction): number {
        if (transaction && transaction.currency && transaction.users && transaction.totalValue && transaction.totalValue > 0) {
            return transaction.totalValue / this.CountSellers(transaction);
        }
    }

    public static SellerShareValuePlusMargin(transaction: Transaction): number {
        if (transaction && transaction.currency && transaction.users && transaction.totalValue && transaction.totalValue > 0) {
            var sellerShareValue = transaction.totalValue / this.CountSellers(transaction);
            return sellerShareValue + (sellerShareValue / 100) * Config.transaction_margin;
        }
    }
