...
export class AssetMapComponent implements OnInit, OnDestroy {
    ...
    ngAfterContentInit() {
        this.clickService.createInstance(ActionIcon.LIST, CurrentComponent.AssetMapComponent);
        this.clickSubscription = this.clickService.getAssetMapClick().subscribe(() => {
            this.gotoAssetList();
        });
    }
    ...
    ngAfterViewInit() {
        ...
        if (this.assets && this.assets.length > 0) {
            for (var i = 0; i <= this.assets.length; i++) {
                ...
                this.addMarker(location.latitude, location.longitude, asset.title, asset.id);
            }
        }
    }
    ...
    private addMarker(latitude: number, longitude: number, title: string, assetId: string) {
