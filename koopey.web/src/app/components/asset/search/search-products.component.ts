...
@Component({
    ...
    templateUrl: "../../views/search-products.html",
...
export class SearchProductsComponent implements OnInit, OnDestroy {
    ...
    private LOG_HEADER: string = "SearchProductsComponent"
    //Booleans 
    ...
    ngAfterContentInit() {
        this.clickService.createInstance(ActionIcon.SEARCH, CurrentComponent.SearchAssetsComponent);
        this.clickSubscription = this.clickService.getSearchAssetsClick().subscribe(() => {
            this.findAssets();
        });
    }
    ...
    private findAssets() {
        ...
        if (!this.search.latitude && !this.search.longitude) {
            ...
            //Set progress icon
            this.searching = true; console.log(this.search);
