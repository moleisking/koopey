...
export class EventCreateComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.clickService.createInstance(ActionIcon.CREATE, CurrentComponent.EventCreateComponent);
        this.clickSubscription = this.clickService.getEventCreateClick().subscribe(() => {
            this.createEvent();
        });
    }
    ...
    ngAfterContentInit() {
        this.eventSubscription = this.eventService.getEvent().subscribe(
            (event) => { if (event) { this.event = event; } else { this.event = new Event() } },
    ...
    private createEvent() {
        if (Event.isEmpty(this.event)) {
            ...
            this.eventService.createEvent(this.event).subscribe(
                (alert) => { this.alertService.success("INFO_COMPLETE"); },
