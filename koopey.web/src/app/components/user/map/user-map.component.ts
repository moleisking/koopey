...
export class UserMapComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.userSubscription = this.userService.getUsers().subscribe(
            ...
            () => { if (!Config.system_production) { console.log(this.users); } });
    }

    ngAfterContentInit() {
        this.clickService.createInstance(ActionIcon.LIST, CurrentComponent.UserMapComponent);
        this.clickSubscription = this.clickService.getUserMapClick().subscribe(() => {
            this.gotoUserList();
        });
    }
    ...
    ngAfterViewInit() {
        ...
        if (this.users && this.users.length > 0) {
            for (var i = 0; i <= this.users.length; i++) {
                ...
                this.addMarker(location.latitude, location.longitude, user.alias, user.id);
            }
        }
    }
