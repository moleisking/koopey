...
@Component({
    selector: "transaction-map-component",
...
export class TransactionMapComponent implements OnInit, OnDestroy {
    ...
    ngAfterContentInit() {
        this.clickService.createInstance(ActionIcon.LIST, CurrentComponent.TransactionMapComponent);
        this.clickSubscription = this.clickService.getTransactionMapClick().subscribe(() => {
    ...
    ngAfterViewInit() {
        //Create map object
        var mapProp = {
            center: new google.maps.LatLng(Config.default_latitude, Config.default_longitude),
            zoom: 5,
        ...
        if (this.transactions && this.transactions.length > 0) {
            for (var i = 0; i <= this.transactions.length; i++) {
                ...
                for (var j = 0; j <= transaction.users.length; j++) {
                    ...
                    this.addMarker(transaction.location.latitude, transaction.location.longitude, user.alias, user.id);
                }    
            }
        }
    }
    ...
    private addMarker(latitude: number, longitude: number, title: string, userId: string) {
