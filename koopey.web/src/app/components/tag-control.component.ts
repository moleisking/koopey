...
export class TagControlComponent implements OnInit, ControlValueAccessor, OnChanges {
    ...
    writeValue(value: any) {
        if (!this.readOnly) {
            ...
            if (value) { //if (value !== this.address) {
    ...
    filterTags(value: string) {
        if (!this.readOnly) {
            if (value) {
                return value ? this.tags.filter(tag => new RegExp(`^${value}`, 'gi').test(this.getTagText(tag))) : this.tags;
            }
        }
    }
    ...
    private getTags() {
        this.tagService.readAll().subscribe(
            ...
            error => { console.log("getTags error:" + error); }
        );
    }
