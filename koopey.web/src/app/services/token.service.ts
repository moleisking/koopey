...
export class TokenService {
    ...
    public createSystemReceiptFromInvoice(token: Token): Observable<Alert> {
        ...
        let options = new RequestOptions({ 'headers': headers });
        let body = JSON.stringify(token);
        var url = Config.system_backend_url + "/token/create/system/receipt/from/invoice";
        return this.http.post(url, body, options).map((res: Response) => { return res.json().alert }).catch(this.handleError);
    }
    ...
    public readHash(): Observable<Token> {
        ...
        return this.http.get(url, options).map((res: Response) => { return res.json().token }).catch(this.handleError);
    }
    ...
    private handleError(error: any) {
        return Observable.throw({ "TokenService:": { "Code": error.status, "Message": error.message } });
    }
}